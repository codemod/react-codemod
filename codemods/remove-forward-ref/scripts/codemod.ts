...
function signaturePrefix(fn: SgNode<TSX>, paramsText: string): string {
  const source = fn.text();
  const paramsIndex = source.indexOf(paramsText);
  return paramsIndex >= 0 ? source.slice(0, paramsIndex) : "";
}

function signatureBetweenParamsAndBody(
  fn: SgNode<TSX>,
  paramsText: string,
  fnBodyText: string,
): string {
  const source = fn.text();
  const paramsIndex = source.indexOf(paramsText);
  const bodyIndex = source.lastIndexOf(fnBodyText);
  if (paramsIndex < 0 || bodyIndex < 0 || bodyIndex < paramsIndex) {
    return "";
  }

  return source.slice(paramsIndex + paramsText.length, bodyIndex);
}
...
function buildReplacement(call: SgNode<TSX>): string | null {
  ...
  const paramsText = renderFunction.field("parameters")?.text() ?? "()";
  const prefix = signaturePrefix(renderFunction, paramsText);
  const betweenParamsAndBody = signatureBetweenParamsAndBody(renderFunction, paramsText, fnBodyText);

  return `${prefix}(${finalParam})${betweenParamsAndBody}${fnBodyText}`;
}
