  ...
  for (const assign of assignments) {
    ...
    if (willBeEmptyConstructor && constructorNode) {
      ...
      if (canRemoveSelfDeclaration && selfDeclaration) {
        const selfEnd = selfDeclaration.range().end.index;
        const afterSelf = source.slice(selfEnd, selfEnd + 10);
        const selfTrailing = afterSelf.match(/^\s*\n?/)?.[0] ?? "\n";
        edits.push({
          startPos: selfDeclaration.range().start.index,
          endPos: selfEnd + selfTrailing.length,
          insertedText: "",
        });
      }
    }
